...
//! This crate is a hack so build.rs for cxx-qt-core-lib and cxx-qt-build both have access to cxx-qt-core-lib's C++ headers.
//! This must be a separate crate from cxx-qt-core-lib because cxx-qt-core-lib cannot be a build dependency of cxx-qt-build.
...
/// Write the cxx-qt-core-lib headers to the specified directory.
pub fn write_headers(directory: impl AsRef<Path>) {
    let directory = directory.as_ref();
    std::fs::create_dir_all(directory).expect("Could not create cxx-qt-core-lib header directory");
    for (file_contents, file_name) in [
        ...
        let mut header = File::create(h_path).expect("Could not create cxx-qt-core-lib header");
        write!(header, "{file_contents}").expect("Could not write cxx-qt-core-lib header");
    }
}
